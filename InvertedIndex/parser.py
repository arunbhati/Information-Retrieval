...
def p_expression_query(p):
	'''query    : QUOTES TERMS QUOTES 
				| TERMS'''
	if(p[1]=='"'):
		temp_class=query_evaluation("document.pickle","dictionary.pickle","posting_list")
		term_list=[]
		for i in p[2].split():
			if(i!=' '):
				term_list.append(i)
		ans_list=temp_class.phrasalquery(term_list,0)
		p[0]=sorted(ans_list, key=lambda x: x.score)
	else:
		temp_class=query_evaluation("document.pickle","dictionary.pickle","posting_list")
		term_list=[]
		for i in p[1].split():
			if(i!=' '):
				term_list.append(i)
		print term_list
		ans_list=temp_class.multiquery(term_list,0)
		p[0]=sorted(ans_list, key=lambda x: x.score)
		#print p[0]
